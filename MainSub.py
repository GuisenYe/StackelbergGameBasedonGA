...
def gurobiFunction(price,purchasingPrice):
    ...
    IDCCost = Model.addVars(24, vtype=GRB.CONTINUOUS, name="IDCCost")
    ...
    # # ADD CONSTRAINT
    # Model.addConstr(IDCCost.sum()<=16753)
    # Model.addConstr(IDCCost.sum()>=16700)

    for i in range(24):

        Model.addConstr(IDCCost[i]==PgridA[i] * CAPrice[i] + UnitCostA1[i] + UnitCostA2[i],"cost ")
